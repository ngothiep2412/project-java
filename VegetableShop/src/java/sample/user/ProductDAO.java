...
public class ProductDAO {
    ...
    private static final String SEARCH = "SELECT [productID]\n"
            ...
            + "                                   ON [tblProduct].[categoryID] = [tblCategory].[categoryID] WHERE [productName] LIKE ? AND [status] = 1\n"
            + "					ORDER BY [productID]\n"
            + "                                    OFFSET (? - 1) * ? ROW FETCH NEXT ? ROWS ONLY";
    private static final String SEARCH_CATEGORY = "SELECT [productID]\n"
            + "                                     ,[productName]\n"
            + "                                        ,[quantity]\n"
            + "                                     ,[price]\n"
            + "                                     ,[description]\n"
            ...
            +"                                          ,[status]\n"
            + "                                    FROM [dbo].[tblProduct] INNER JOIN [tblCategory]\n"
            + "                                   ON [tblProduct].[categoryID] = [tblCategory].[categoryID] WHERE [tblProduct].[categoryID] LIKE ? AND [status] = 1\n"
            + "					ORDER BY [productID]\n"
            + "                                    OFFSET (? - 1) * ? ROW FETCH NEXT ? ROWS ONLY";
    private static final String DELETE = "UPDATE [dbo].[tblProduct]\n"
            + "   SET [status] = 0 WHERE [productID] = ?";

    public List<ProductDTO> searchByAdminWithPaging(String search, int page, int PAGE_SIZE) throws SQLException {
    ...
    public List<ProductDTO> getProductsByCategoryID(int categoryID) throws SQLException {
        ...
        try {
            String sql = "SELECT [productID]\n"
                    ...
                    + "                         ,[status]\n"
                    + "                    FROM [dbo].[tblProduct] INNER JOIN [tblCategory]\n"
                    + "                    ON [tblProduct].[categoryID] = [tblCategory].[categoryID] WHERE [tblProduct].[categoryID] = ? AND [status] = 1";
            conn = DBUtils.getConnection();
            if (conn != null) {
    ...
    public List<ProductDTO> getListProductWithPaging(int page, int PAGE_SIZE) throws SQLException {
        ...
        try {
            String sql = "SELECT [productID]\n"
                    ...
                    + "                        ,[status]\n"
                    + "                    FROM [dbo].[tblProduct] INNER JOIN [tblCategory]\n"
                    + "                    ON [tblProduct].[categoryID] = [tblCategory].[categoryID] WHERE [status] = 1"
                    + "     ORDER BY [productID]\n"
                    + "                    OFFSET (? - 1) * ? ROW FETCH NEXT ? ROWS ONLY";
            conn = DBUtils.getConnection();
            if (conn != null) {
    ...
    public ProductDTO geProductByProductID(int productID) throws SQLException {
        ...
        try {
            String sql = "SELECT [productID]\n"
                    ...
                    + "      ,[usingDate]\n"
                    +"       ,[status]\n"
                    + "FROM [dbo].[tblProduct] WHERE [productID] = ? AND [status] = 1";
            conn = DBUtils.getConnection();
            if (conn != null) {
                ptm = conn.prepareStatement(sql);
                ptm.setInt(1, productID);
    ...
    public int getTotalProductsWithKeyWord(String search) throws SQLException {
        Connection conn = null;
        PreparedStatement ptm = null;
        ResultSet rs = null;
        try {
            String sql = "SELECT COUNT(productID) FROM [dbo].[tblProduct] WHERE [productName] LIKE ? AND [status] = 1";
            conn = DBUtils.getConnection();
            if (conn != null) {
                ptm = conn.prepareStatement(sql);
                ptm.setString(1, "%" + search + "%");
    ...
    public int getTotalProductsByCategoryID(int categoryID) throws SQLException {
        Connection conn = null;
        PreparedStatement ptm = null;
        ResultSet rs = null;
        try {
    ...
    public List<ProductDTO> getProductsByCategoryIDWithPaging(int categoryID, int page, int PAGE_SIZE) throws SQLException {
        List<ProductDTO> list = new ArrayList<>();
        Connection conn = null;
        PreparedStatement ptm = null;
        ResultSet rs = null;
    ...
    public List<ProductDTO> getListProduct() throws SQLException {
        ...
        try {
            String sql = "SELECT [productID]\n"
                    ...
                    + "      ,[usingDate]\n"
                    +"       ,[status]\n"
                    + "FROM [dbo].[tblProduct] WHERE [status] = 1";
            conn = DBUtils.getConnection();
            if (conn != null) {
                ptm = conn.prepareStatement(sql);
                rs = ptm.executeQuery();
